...
const validateEmail = (email) =>
  // Solution code here...
  /((\w+[.]\w+)|\w+)[@]\w+(.com\b|.net\b|.org\b)/.test(email);
...
const validatePhoneNumber = (phoneNumber) =>
  // Solution code here...
  /((([(]\d{3}[)])|(\d{3}[-])|(\d{3}[ ]))\d{3}[ ]\d{4})|(((\d{3,6})|([(]\d{3}[)]))([ ]|[-])((\d{3}[-]\d{4})|\d{4,7}))|(\d{10})/.test(phoneNumber);
...
const findTagNames = elements =>{
  // Solution code here...
  const matches = [];
  (elements.map(element => element.match(/\/\w+/g))).map(array => array.map(string => matches.push(string)));
  return matches;
};
