...
const totalSum = (input) =>
  // Solution code here...
  input.map(array => array.reduce((acc, cVal) => {
    acc += cVal;
    return acc;
  })).reduce((acc, cVal) => {
    acc += cVal;
...
const divisibleByFiveTwoToThePower = (input) =>
  // Solution code here...
  input.map(array => array.filter(element => typeof(element) === 'number' && element%5===0)).map(array => array.map(number => Math.pow(2, number)));
...
let findShortest = (data) => {
  // Solution code here...
  return data.find(character => character.height === (Math.min(...data.map(character => parseInt(character.height)))).toString()).name;
};
